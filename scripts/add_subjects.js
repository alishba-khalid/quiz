...
const NEW_SUBJECTS = [
    {
        ...
        faq: [
            { q: "Can I generate questions for a specific book?", a: "Yes. Include the book title and chapter in your prompt, e.g., 'To Kill a Mockingbird Chapter 3, Grade 9.'" },
...
function toTs(val, indent = 0) {
    ...
    if (Array.isArray(val)) {
        ...
        if (val.every(x => typeof x === "string" || typeof x === "number") && val.join(", ").length < 60) {
            return "[" + val.map(x => toTs(x)).join(", ") + "]";
        }
        const parts = val.map(x => ind + "  " + toTs(x, indent + 2).trimStart());
        return "[\n" + parts.join(",\n") + "\n" + ind + "]";
    }
    if (typeof val === "object") {
        const parts = Object.entries(val).map(([k, v]) => {
            const key_str = k.match(/^[a-zA-Z0-9]+$/) ? k : `"${k}"`;
            return ind + "  " + key_str + ": " + toTs(v, indent + 2).trimStart();
        });
        return "{\n" + parts.join(",\n") + "\n" + ind + "}";
    }
    return "null";
...
function main() {
    const targetPath = path.join(__dirname, "..", "src", "lib", "subjects.ts");
    let content = fs.readFileSync(targetPath, "utf8");

    // 1. Add to quizSubjects
    const quizAnchor = "\n];\n\nexport const worksheetSubjects: SubjectData[] = [";
    if (!content.includes(quizAnchor)) {
        console.error("Could not find quizSubjects anchor!");
        process.exit(1);
    }

    const serializedNew = ",\n" + NEW_SUBJECTS.map(s => toTs(s, 2)).join(",\n");
    
    let parts = content.split(quizAnchor);
    content = parts[0] + serializedNew + quizAnchor + parts[1];

    // 2. Add to worksheetSubjects
    const worksheetAnchor = "\n];\n\nexport function getQuizSubject";
    if (!content.includes(worksheetAnchor)) {
        console.error("Could not find worksheetSubjects anchor!");
        process.exit(1);
    }

    parts = content.split(worksheetAnchor);
    content = parts[0] + serializedNew + worksheetAnchor + parts[1];

    fs.writeFileSync(targetPath, content, "utf8");
    console.log("Successfully added 12 new subjects to quizSubjects and worksheetSubjects!");
}
